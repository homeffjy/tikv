...
fn test_compact_after_delete<T: Simulator>(cluster: &mut Cluster<T>) {
    cluster.cfg.raft_store.enable_file_based_compaction = false;
...
fn test_compact_files_after_delete<T: Simulator>(cluster: &mut Cluster<T>) {
    cluster.cfg.raft_store.enable_file_based_compaction = true;
    cluster.cfg.raft_store.region_compact_check_interval = ReadableDuration::millis(100);
    cluster.cfg.raft_store.region_compact_min_tombstones = 500;
    cluster.cfg.raft_store.region_compact_tombstones_percent = 50;
    cluster.cfg.raft_store.region_compact_redundant_rows_percent = Some(1);
    ...
    let sync_sender = Mutex::new(sender);
    fail::cfg_callback(
        "raftstore::compact::CheckAndCompactFiles:AfterCompact",
        move || {
            let sender = sync_sender.lock().unwrap();
            sender.send(true).unwrap();
        },
...
#[test]
fn test_node_compact_files_after_delete() {
    let count = 1;
    let mut cluster = new_node_cluster(0, count);
    test_compact_files_after_delete(&mut cluster);
}
